...
- For any work related to the `InfoProvider` usermod, switch to `feature/info-scrolling-text` first and make all changes only on that branch.
- For testing, merge the changes into `feature/all` and compile there; do not make InfoProvider implementation changes directly on `feature/all`.
...
- **DMA compatibility**: on ESP32 (classic), PSRAM buffers are **not DMA-capable**. On ESP32-S3 with octal PSRAM (`CONFIG_SPIRAM_MODE_OCT`), PSRAM buffers _can_ be used with DMA when `CONFIG_SOC_PSRAM_DMA_CAPABLE` is defined.
...
- **Performance**: Prefer DRAM (or IRAM) for hot-path data that is _frequently_ used. Prefer PSRAM for capacity-oriented buffers where slightly slower access times can be tolerated.
