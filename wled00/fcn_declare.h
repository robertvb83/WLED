...
typedef struct WiFiConfig
{
  ...
  WiFiConfig(const char *ssid = "", const char *pass = "", uint32_t ip = 0, uint32_t gw = 0, uint32_t subnet = 0x00FFFFFF) // little endian
#endif
      : staticIP(ip), staticGW(gw), staticSN(subnet)
  {
    strncpy(clientSSID, ssid, 32);
    clientSSID[32] = 0;
    strncpy(clientPass, pass, 64);
    clientPass[64] = 0;
#ifdef WLED_ENABLE_WPA_ENTERPRISE
    encryptionType = enc_type;
    strncpy(enterpriseAnonIdentity, ent_anon, 64);
    enterpriseAnonIdentity[64] = 0;
    strncpy(enterpriseIdentity, ent_iden, 64);
    enterpriseIdentity[64] = 0;
#endif
...
struct scroll_info_data_t
{
  char text[8][WLED_MAX_SEGNAME_LEN + 1];
  uint32_t color[8];
  bool valid[8];
};
...
protected:
  ...
  template <typename T>
  static inline void oappend(const T &t) { oappend_shim->print(t); };
#ifdef ESP8266
  // Handle print(PSTR()) without crashing by detecting PROGMEM strings
  static void oappend(const char *c)
  {
    if ((intptr_t)c >= 0x40000000)
      oappend_shim->print(FPSTR(c));
    else
      oappend_shim->print(c);
  };
#endif
};
...
#define hex2int(a) (((a) >= '0' && (a) <= '9') ? (a) - '0' : ((a) >= 'A' && (a) <= 'F') ? (a) - 'A' + 10 \
                                                         : ((a) >= 'a' && (a) <= 'f')   ? (a) - 'a' + 10 \
                                                                                        : 0)
